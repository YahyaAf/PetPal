...
// ─── Helpers ──────────────────────────────────────────────────
const TYPE_CONFIG = {
  HOTEL:    { label: "Hôtel",     icon: "🏨", cls: "bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800"     },
  TRAINING: { label: "Formation", icon: "🤺", cls: "bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 border-green-200 dark:border-green-800"   },
  ORDER:    { label: "Commande",  icon: "🛒", cls: "bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 border-purple-200 dark:border-purple-800" },
};

const TypeBadge = ({ type }) => {
  const cfg = TYPE_CONFIG[type] ?? { label: type, icon: "📋", cls: "bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700" };
  return (
    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 px-5 py-4">
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div className="w-9 h-9 bg-gray-50 dark:bg-gray-800 rounded-xl flex items-center justify-center text-base shrink-0">
            {TYPE_CONFIG[review.reservationType]?.icon ?? "📋"}
          </div>
          <div className="min-w-0">
            <p className="text-sm font-bold text-gray-800 dark:text-gray-200 truncate">{review.userNom || "—"}</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 truncate">{review.userEmail || "—"}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-gray-400 dark:text-gray-500">{formatDate(review.dateReview)}</span>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            disabled={deleting}
            className="text-xs px-3 py-1.5 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 rounded-lg transition-colors font-medium disabled:opacity-50"
          >
            {deleting ? "…" : "🗑️"}
          </button>
        </div>
      ...
      {/* Expanded */}
      {expanded && (
        <div className="border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 px-5 py-4 space-y-3">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
            <div>
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-0.5">ID avis</p>
              <p className="font-mono font-semibold text-gray-800 dark:text-gray-200">#{review.idReview}</p>
            </div>
            <div>
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-0.5">Réf. réservation</p>
              <p className="font-mono font-semibold text-gray-800 dark:text-gray-200">#{review.reviewId}</p>
            </div>
            <div>
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-0.5">Note</p>
              <StarDisplay value={review.rating} size="text-base" />
            </div>
            <div>
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-0.5">Date</p>
              <p className="font-medium text-gray-800 dark:text-gray-200">{formatDate(review.dateReview)}</p>
            </div>
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4">
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1.5">Commentaire</p>
            <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{review.commentaire}</p>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Avis clients</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Gestion de tous les avis</p>
        </div>
        <button
          onClick={fetchAll}
      ...
      {!loading && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: "Note moy.",  value: `${avgRating} ★`,        cls: "bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400"  },
            { label: "Hôtels",     value: countByType("HOTEL"),    cls: "bg-sky-50 dark:bg-sky-900/20 text-sky-700 dark:text-sky-400"    },
            { label: "Formations", value: countByType("TRAINING"), cls: "bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400"  },
          ].map(({ label, value, cls }) => (
