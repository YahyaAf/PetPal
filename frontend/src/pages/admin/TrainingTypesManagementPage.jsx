  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">Types de dressage</h1>
        <button
          onClick={openCreate}
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">Durée</th>
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
              {trainingTypes.length === 0 ? (
                <tr>
                  <td colSpan={6} className="text-center py-12 text-gray-400 dark:text-gray-600">Aucun type de dressage trouvé</td>
                </tr>
              ) : (
                trainingTypes.map((t) => (
                  <tr key={t.idType} className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                    <td className="px-4 py-3 text-gray-400 dark:text-gray-600">{t.idType}</td>
                    <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-200">{t.nom}</td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400 max-w-55 truncate" title={t.description}>
                      {t.description || <span className="text-gray-300 dark:text-gray-600">—</span>}
                    </td>
                    <td className="px-4 py-3 font-medium text-gray-700 dark:text-gray-300">{t.prix} MAD</td>
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400">
