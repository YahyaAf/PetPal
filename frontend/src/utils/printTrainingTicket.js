/**
 * Generates and opens a printable/downloadable training reservation ticket
 * with PetPal branding — Black/Gold elegant design.
 * Same design logic as printHotelTicket but for training reservations
 */
export const printTrainingTicket = (reservation, user = {}) => {
  const trainingType = reservation.trainingType || {};
  const trainingNom = trainingType.nom || trainingType.name || "Formation";
  const dresseur = reservation.dresseur || {};
  const dresseurNom = dresseur.nom || dresseur.name || "Dresseur";
  const dresseurEmail = dresseur.email || "";

  const resId = `FORM-${String(reservation.idReservation ?? 0).padStart(5, "0")}`;
  const montant = Number(reservation.totalPrice ?? 0).toFixed(2);
  const duree = reservation.duree ?? trainingType.duree ?? "1";
  ...
  const debut = formatDate(reservation.dateDebut);
  const fin = formatDate(reservation.dateFin);
    ...
    <div class="ticket-content">
      ...
      <div class="ticket-left">
        <div>
          ...
          <div class="training-title">${trainingNom}</div>
          <div class="training-subtitle">Formation pour animaux PetPal</div>
        </div>

        <div class="details-section">
          <div class="detail-block">
            <div class="detail-label">Dates</div>
            <div class="detail-content">${debut} → ${fin}</div>
          </div>

          <div class="detail-block">
            <div class="detail-label">Durée</div>
            <div class="detail-content">${duree} jour${Number(duree) > 1 ? "s" : ""}</div>
          </div>
          ...
          <div class="trainer-section">
            <div class="trainer-label">Dresseur</div>
            <div class="trainer-name">${dresseurNom}</div>
            ${dresseurEmail ? `<div class="trainer-email">${dresseurEmail}</div>` : ""}
          </div>
        </div>
      </div>
      ...
      <div class="ticket-right">
        ...
        <div class="training-name-right">${trainingNom}</div>
        <div class="year">${currentYear}</div>

        <div class="ticket-details">
          <div class="ticket-detail-item">
            <div class="ticket-detail-label">Numéro de formation</div>
